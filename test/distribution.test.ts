...
describe('creates kubeconform distribution', () => {
    ...
    describe('for Linux', () => {
        ...
        it('with arch x32', async () => {
            ...
            expect(dist.getDownloadURL()).toBe(
                'https://github.com/yannh/kubeconform/releases/download/v0.8.0/kubeconform-linux-386.tar.gz'
            )
        })
        ...
        it('with arch armv6', async () => {
            ...
            expect(dist.getDownloadURL()).toBe(
                'https://github.com/yannh/kubeconform/releases/download/v0.8.0/kubeconform-linux-armv6.tar.gz'
            )
        })
    })
    ...
    describe('for Windows', () => {
        ...
        it('with arch x32', async () => {
            const dist = new distribution.KubeconformDistribution(version, os, 'x32')
            expect(dist.getDownloadURL()).toBe(
                'https://github.com/yannh/kubeconform/releases/download/v0.8.0/kubeconform-windows-386.zip'
            )
        })
        it('with arch x64', async () => {
            const dist = new distribution.KubeconformDistribution(version, os, 'x64')
            expect(dist.getDownloadURL()).toBe(
                'https://github.com/yannh/kubeconform/releases/download/v0.8.0/kubeconform-windows-amd64.zip'
            )
        })
        it('with arch arm64', async () => {
            const dist = new distribution.KubeconformDistribution(version, os, 'arm64')
            expect(dist.getDownloadURL()).toBe(
                'https://github.com/yannh/kubeconform/releases/download/v0.8.0/kubeconform-windows-arm64.zip'
            )
        })
    })
})
